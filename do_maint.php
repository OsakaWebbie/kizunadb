...
<form action="<? echo $PHP_SELF; ?>" method="post">
  <input type="hidden" name="catid" value="<? echo $_POST['catid']; ?>">
  <input type="hidden" name="cat_del" value="<? echo $cat_del; ?>">
...
?>
<h3><font color="red"><? echo _("Please Confirm Contact Type Delete"); ?></font></h3>
<? printf(_("There are %s contact entries of this contact type.  If you delete this contact type, I must assign a different contact type to those contact entries."),$ct_num);
...
<form action="<? echo $PHP_SELF; ?>" method="post">
  <p><? echo _("New Contact Type"); ?>:
    <select size="1" name="new_ctypeid">
      <option value=""><? echo _("Select..."); ?></option><? echo $options; ?>
    </select>
  <input type="hidden" name="ctypeid" value="<? echo $_POST['ctypeid']; ?>">
  <input type="hidden" name="ct_del" value="<? echo $_POST['ct_del']; ?>">
  <input type="hidden" name="confirmed" value="1">
  <input type="submit" value="<? echo _("Yes, reassign the contact entries"); ?>">
</form>
<?
    $need_confirmation = 1;
  }
...
?>
<h3><font color="red"><? echo _("Please Confirm Donation Type Delete"); ?></font></h3>
<? printf(_("There are %s donation entries of this donation type.  If you delete this donation type, I must assign a different donation type to those donation entries."),$dt_num);
...
<form action="<? echo $PHP_SELF; ?>" method="post">
  <p><? echo _("New Donation Type"); ?>:
    <select size="1" name="new_dtypeid">
      <option value=""><? echo _("Select..."); ?></option><? echo $options; ?>
    </select>
  <input type="hidden" name="dtypeid" value="<? echo $_POST['dtypeid']; ?>">
  <input type="hidden" name="dt_del" value="<? echo $_POST['dt_del']; ?>">
  <input type="hidden" name="confirmed" value="1">
  <input type="submit" value="<? echo _("Yes, reassign the donation entries"); ?>">
</form>
<?
    $need_confirmation = 1;
  }
...
// ********** EVENT  **********
  ...
  if ($confirmed) {
    ...
    if (mysql_affected_rows() > 0) {
      $message = sprintf(_("%s related attendance records deleted."),mysql_affected_rows())."\\n";
    }
...
<form action="<? echo $PHP_SELF; ?>" method="post">
  ...
  <input type="hidden" name="event_del" value="<? echo $event_del; ?>">
  <input type="hidden" name="confirmed" value="1">
  <input type="submit" value="<? echo _("Yes, delete the event and attendance records"); ?>">
</form>
<?
    $need_confirmation = 1;
  }
...
// ********** LOGIN  **********
  ...
  if ($_POST['userid'] == "new") {
    ...
    $result = sqlquery_checked("SELECT UserName FROM login WHERE UserID='".$_POST['new_userid']."'");
    if ($new_userid != $old_userid && mysql_numrows($result) > 0) {
      $row = mysql_fetch_object($result);
      $message = sprintf(_("UserID '%s' is already in use by %s. Please choose a different UserID."),
